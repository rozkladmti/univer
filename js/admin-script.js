...
let showAllDepartments = false; // Режим показу всіх кафедр (тільки для адміна)
...
function getDepartmentName(code) {
    ...
    console.log('[GET DEPT NAME] Looking for code:', code);
    console.log('[GET DEPT NAME] dictionaries.departments:', dictionaries?.departments?.length);
    ...
    if (dictionaries && dictionaries.departments && Array.isArray(dictionaries.departments)) {
        ...
        console.log('[GET DEPT NAME] Found dept:', dept);
...
/**
 * Підрахунок заявок по авторах (викладач або кафедра)
 */
function countRequestsByAuthor() {
    const counts = {};
    
    console.log('[COUNT AUTHORS] teacherRequests:', teacherRequests?.length);
    console.log('[COUNT AUTHORS] draftLessons:', draftLessons?.length);
    
    // Рахуємо teacher requests - ТОЧНО ТА САМА ЛОГІКА ЩО І ДЛЯ BADGE
    if (teacherRequests && teacherRequests.length > 0) {
        teacherRequests.forEach(request => {
            ...
            // ТА САМА ЛОГІКА ЩО НА РЯДКУ 2546 для deptCode
            ...
            // ТА САМА ЛОГІКА ЩО НА РЯДКУ 2561 для _requestAuthor
            const author = request.teacher || deptCode;
            
            const source = request.source || 'teacher';
            
            console.log('[COUNT AUTHORS] Final author:', author, 'source:', source);
            
            if (!author || author === 'UNKNOWN') {
                console.warn('[COUNT AUTHORS] Skipping request with no author:', request.id);
                return;
            }
    ...
    // Рахуємо draft proposals - просто всі які є
    if (draftLessons && Array.isArray(draftLessons) && draftLessons.length > 0) {
        const draftsByDept = {};
        
        draftLessons.forEach(lesson => {
            const deptCode = lesson.departmentcode;
            if (!deptCode) return;
            
            console.log('[COUNT AUTHORS] Draft lesson dept:', deptCode, 'id:', lesson.id);
            
            draftsByDept[deptCode] = (draftsByDept[deptCode] || 0) + 1;
        });
        
        // Додаємо до counts
        Object.keys(draftsByDept).forEach(deptCode => {
            if (!counts[deptCode]) {
                counts[deptCode] = { 
                    count: 0, 
                    source: 'department',
            ...
            counts[deptCode].count += draftsByDept[deptCode];
        });
    } else {
        console.log('[COUNT AUTHORS] No draft lessons to count');
    }
    
    console.log('[COUNT AUTHORS] Final counts:', counts);
...
function renderDepartmentFilterPanel() {
    if (!isCompareMode) {
        console.log('[AUTHOR FILTER] Not in compare mode');
        return '';
    }
    ...
    console.log('[AUTHOR FILTER] Counts:', authorCounts, 'Total:', totalRequests);
...
function applyDepartmentFilter() {
    ...
    // Проходимо по всіх картках і ховаємо/показуємо залежно від автора
    document.querySelectorAll('.lesson-card').forEach(card => {
        // Беремо автора з dataset
        ...
        // Застосовуємо фільтр
        if (selectedDepartmentFilter === 'ALL') {
            // Показуємо всі крім UNKNOWN (це context картки без змін)
            if (author === 'UNKNOWN' && !isContext) {
            ...
            card.classList.remove('dept-hidden');
            card.style.display = '';
        } else if (author === selectedDepartmentFilter) {
            // Показуємо картки обраного автора
            card.classList.remove('dept-hidden');
            card.style.display = '';
        } else {
            // Ховаємо картки інших авторів
            card.classList.add('dept-hidden');
            card.style.display = 'none';
        }
    });
...
async function loadDictionaries() {
    try {
        ...
        if (result.result === 'success') {
            ...
            // Завантажуємо збережений вибір кафедри
            loadDepartmentChoice();
            
            // Якщо не обрано - ставимо першу
            if (!currentDepartment.departmentCode && dictionaries.departments.length > 0) {
                const firstDept = dictionaries.departments[0];
                currentDepartment.departmentCode = firstDept.code;
                currentDepartment.facultyCode = firstDept.facultyCode;
            }
...
// Функції для отримання ВСІХ даних (для адміна)
function getAllTeachers() {
    return dictionaries.teachers.map(t => t.name);
}

function getAllSubjects() {
    return dictionaries.subjects.map(s => s.name);
}

function getAllRooms() {
    return dictionaries.workshops.map(r => r.name);
}

function updateListsFromDictionaries() {
    // Адмін бачить всі пари всіх кафедр, тому списки повинні бути повні
...
function filterLessonsByDepartment(allLessons) {
    // Якщо адмін увімкнув режим "всі кафедри" - не фільтруємо
    if (showAllDepartments) return allLessons;
    
    // 3. Якщо нічого не обрано (немає facultyCode і departmentCode) → показуємо весь університет
    if (!currentDepartment.facultyCode && !currentDepartment.departmentCode) {
        return allLessons;
    }
    
    // 1. Якщо обрано ТІЛЬКИ факультет (немає departmentCode) → показуємо всі кафедри цього факультету
    if (currentDepartment.facultyCode && !currentDepartment.departmentCode) {
        // Отримуємо всі кафедри цього факультету
        const facultyDepartments = dictionaries.departments?.filter(dept => 
            dept.facultyCode === currentDepartment.facultyCode
        ) || [];
        
        const facultyDepartmentCodes = facultyDepartments.map(dept => dept.code);
        
        // Збираємо всіх викладачів з усіх кафедр факультету (один раз, для оптимізації)
        const facultyTeachers = new Set();
        facultyDepartments.forEach(dept => {
            const deptTeachers = getTeachersForDepartment(dept.code);
            deptTeachers.forEach(teacher => facultyTeachers.add(teacher));
        });
        
        // Фільтруємо уроки по кафедрах цього факультету
        return allLessons.filter(lesson => {
            // Якщо є departmentcode - перевіряємо чи він в списку кафедр факультету
            if (lesson.departmentcode) {
                return facultyDepartmentCodes.includes(lesson.departmentcode);
            }
            
            // Якщо немає departmentcode - визначаємо по викладачу (зворотна сумісність)
            // Перевіряємо чи викладач належить до будь-якої кафедри цього факультету
            return facultyTeachers.has(lesson.teacher) || 
                   (lesson.teacher2 && facultyTeachers.has(lesson.teacher2));
        });
    }
    
    // 2. Якщо обрано факультет + кафедру → показуємо тільки цю кафедру
    if (currentDepartment.departmentCode) {
        // Фільтруємо по departmentcode (lowercase з Google Sheets)
        return allLessons.filter(lesson => {
            // Якщо є departmentcode - використовуємо його
            if (lesson.departmentcode) {
                return lesson.departmentcode === currentDepartment.departmentCode;
            }
            
            // Якщо немає - визначаємо по викладачу (зворотна сумісність)
    ...
    // Fallback - якщо щось не так, показуємо все
    return allLessons;
}
...
function saveDepartmentChoice() {
    localStorage.setItem('currentDepartment', JSON.stringify(currentDepartment));
}

function loadDepartmentChoice() {
    const saved = localStorage.getItem('currentDepartment');
    if (saved) {
        try {
            currentDepartment = JSON.parse(saved);
        } catch (e) {
            console.error('[DEPARTMENT] Error loading:', e);
        }
    }
}
...
function updateNavigationUI() {
    ...
    if (facultySelect) {
        ...
        dictionaries.faculties.forEach(f => {
            const option = document.createElement('option');
            option.value = f.code;
            option.textContent = f.name;
            if (f.code === currentDepartment.facultyCode) option.selected = true;
            facultySelect.appendChild(option);
        });
    }
    
    if (currentDepartment.facultyCode) {
        changeFaculty();
        const deptSelect = document.getElementById('departmentSelect');
        if (deptSelect && currentDepartment.departmentCode) {
            deptSelect.value = currentDepartment.departmentCode;
        }
    }
}
...
function changeFaculty() {
    ...
    const facultyCode = select.value;
    
    currentDepartment.facultyCode = facultyCode;
    currentDepartment.departmentCode = null; // Очищаємо вибір кафедри при зміні факультету
    
    const deptSelect = document.getElementById('departmentSelect');
    deptSelect.innerHTML = '<option value="">Оберіть кафедру</option>';
    
    if (facultyCode) {
        const departments = dictionaries.departments.filter(d => d.facultyCode === facultyCode);
    ...
    saveDepartmentChoice();
    updateListsFromDictionaries();
    renderLessons(); // Оновлюємо розклад одразу при зміні факультету
    // Перевіряємо конфлікти одразу після зміни
    setTimeout(() => {
        checkConflicts();
    }, 50);
}
...
function changeDepartment() {
    ...
    currentDepartment.departmentCode = select.value;
    
    saveDepartmentChoice();
    updateListsFromDictionaries();
    renderLessons();
    // Перевіряємо конфлікти одразу після зміни кафедри
    setTimeout(() => {
        checkConflicts();
    }, 50);
}

function toggleShowAllDepartments() {
    showAllDepartments = !showAllDepartments;
    const btn = document.getElementById('showAllDepartmentsBtn');
    
    if (showAllDepartments) {
        btn.classList.add('active');
        btn.style.background = '#dbeafe';
        btn.style.color = '#1e40af';
        btn.innerHTML = '<i class="fa-solid fa-building-columns"></i> Поточна кафедра';
        //btn.title = 'Показати поточну кафедру';
    } else {
        btn.classList.remove('active');
        btn.style.background = '';
        btn.style.color = '';
        btn.innerHTML = '<i class="fa-solid fa-building-columns"></i> Всі кафедри';
        //btn.title = 'Показати пари всіх кафедр';
    }
    
    renderLessons();
    // Перевіряємо конфлікти одразу після зміни режиму
    setTimeout(() => {
        checkConflicts();
    }, 50);
}
...
function setAdminMode(val) {
    ...
    badge.innerHTML = '<i class="fa-solid fa-user-shield"></i> Суперадмін';
    
    // Показуємо кнопку "Всі кафедри" для адміна
    const showAllBtn = document.getElementById('showAllDepartmentsBtn');
    if (showAllBtn) {
...
async function loadFromGoogle(options = {}) {
    ...
    try {
        ...
        if (data.currentWeek) {
            ...
            // *** ВИПРАВЛЕННЯ: Встановлюємо currentWeek = actualCurrentWeek ***
            // Це гарантує що розклад відкриється на поточному тижні
            currentWeek = actualCurrentWeek;
            
            updateCurrentWeekIndicator(isNextWeek);
        }
        
        // Завантажуємо заявки від викладачів — суперадмін бачить тільки зовнішні (external)
        const allRequests = data.requests || [];
        teacherRequests = allRequests.filter(r => (r.requestType || r.request_type || 'internal') === 'external');
        ...
        // *** ВИПРАВЛЕННЯ: Викликаємо setWeek() щоб правильно встановити активний тиждень ***
        // Це оновить кнопки тижнів та відрендерить розклад для правильного тижня
        setWeek(currentWeek);
...
async function saveMergedChanges() {
    ...
    try {
        ...
        // 6. Оновлюємо teacherRequests локально (серверна частина оновлюється через approve/reject)
        // Requests автоматично видаляються на сервері при approve_request/reject_request
        // Тому не потрібно окремо їх оновлювати
        /*
        ...
        */
        ...
        // Оновлюємо локальний стан teacherRequests
        teacherRequests = updatedRequests;
        console.log('[SAVE MERGED] Локально залишилось requests:', updatedRequests.length);
            ...
            if (draftL && draftL._deleted) {
                ...
                if (liveL) {
                    ...
                    displayList.push({ 
                        ...liveL, 
                        _status: 'removed',
                        _requestAuthor: draftL.departmentcode || liveL.departmentcode || 'UNKNOWN',
                        _requestSource: 'department'
                    });
                }
                ...
                displayList.push({ 
                    ...draftL, 
                    _status: 'added',
                    _requestAuthor: draftL.departmentcode || 'UNKNOWN',
                    _requestSource: 'department'
                });
            }
            else if (liveL && !draftL) {
        ...
        teacherRequests.forEach(req => {
            ...
            req.variants.forEach((variant, idx) => {
                ...
                const ghostLesson = {
                    ...
                    slot: variant.slot,
                    week: variantWeek,
                    departmentcode: deptCode, // Додаємо departmentcode для групування
                    _isRequestGhost: true,
                    _requestId: req.id,
                    _variantIndex: idx,
                    _priority: variant.priority || (idx + 1),
                    _status: 'added',  // Показуємо як нові пари (зелені)
                    _requestDepartment: deptCode, // Зберігаємо для візуалізації та фільтрації
                    _requestAuthor: req.teacher || deptCode, // Автор заявки: викладач або код кафедри
                    _requestSource: req.source || 'teacher' // Джерело: teacher/department
                };
...
function createCard(lesson, doDiff = false) {
    ...
    // Додаємо автора для фільтрації (викладач або кафедра)
    // Для context/moved-source карток використовуємо їх власний departmentcode
    let authorForCard = lesson._requestAuthor;
    if (!authorForCard || authorForCard === 'UNKNOWN') {
        authorForCard = deptCodeForCard;
    }
    div.dataset.author = authorForCard;
    ...
    // Додаємо department badge для пропозицій в режимі порівняння
    let deptBadgeHtml = '';
    if (doDiff && lesson._requestAuthor && (lesson._isRequestGhost || lesson._status === 'added' || lesson._status === 'changed' || lesson._status === 'removed')) {
...
function exportToExcel() {
    try {
        ...
        XLSX.writeFile(wb, `Розклад_ЛНУВМБ_${date}.xlsx`);
...
function openLinksModal() {
    ...
    const searchInput = document.getElementById('facultyDeptSearch');
    ...
    // Рендеримо accordion з факультетами та кафедрами
    renderFacultiesAndDepartments();
    
    document.getElementById('linksModal').classList.add('open');
}

function renderFacultiesAndDepartments(searchTerm = '') {
    const container = document.getElementById('linksListContent');
    container.innerHTML = '';
    
    if (!dictionaries.faculties || dictionaries.faculties.length === 0) {
        container.innerHTML = '<div style="padding: 20px; text-align: center; color: #6b7280;">Список факультетів порожній. Спочатку завантажте дані.</div>';
        return;
    }
    
    if (!dictionaries.departments || dictionaries.departments.length === 0) {
        container.innerHTML = '<div style="padding: 20px; text-align: center; color: #6b7280;">Список кафедр порожній. Спочатку завантажте дані.</div>';
        return;
    }
    ...
    const filteredFaculties = dictionaries.faculties.filter(faculty => {
        if (!searchLower) return true;
        
        const facultyName = faculty.name.toLowerCase();
        const facultyCode = faculty.code.toLowerCase();
        const facultyMatches = facultyName.includes(searchLower) || facultyCode.includes(searchLower);
        
        const facultyDepartments = dictionaries.departments.filter(dept => dept.facultyCode === faculty.code);
        const hasMatchingDept = facultyDepartments.some(dept => {
            const deptName = dept.name.toLowerCase();
            const deptCode = dept.code.toLowerCase();
            return deptName.includes(searchLower) || deptCode.includes(searchLower);
        });
        
        return facultyMatches || hasMatchingDept;
    });
    
    if (filteredFaculties.length === 0) {
        container.innerHTML = '<div style="padding: 20px; text-align: center; color: #6b7280;">Нічого не знайдено</div>';
        return;
    }
    
    filteredFaculties.forEach((faculty, facultyIndex) => {
        const facultyId = `faculty-${faculty.code}`;
        
        const facultyDepartments = dictionaries.departments.filter(dept => {
            if (dept.facultyCode !== faculty.code) return false;
            if (!searchLower) return true;
            const deptName = dept.name.toLowerCase();
            const deptCode = dept.code.toLowerCase();
            return deptName.includes(searchLower) || deptCode.includes(searchLower);
        ...
        const shouldExpand = searchLower && facultyDepartments.length > 0;
        
        const facultyDiv = document.createElement('div');
        facultyDiv.className = 'faculty-accordion-item';
        facultyDiv.style.cssText = 'margin-bottom: 8px; border: 1px solid #e5e7eb; border-radius: 6px; overflow: hidden;';
        
        const facultyHeader = document.createElement('div');
        facultyHeader.className = 'faculty-accordion-header';
        facultyHeader.style.cssText = 'display: flex; justify-content: space-between; align-items: center; padding: 12px 15px; background: #f9fafb; cursor: pointer; user-select: none;';
        facultyHeader.onclick = () => toggleFacultyAccordion(facultyId);
        
        const facultyTitle = document.createElement('span');
        facultyTitle.style.cssText = 'font-weight: 600; font-size: 15px; color: #1f2937;';
        facultyTitle.textContent = faculty.name;
        
        const facultyActions = document.createElement('div');
        facultyActions.style.cssText = 'display: flex; gap: 8px; align-items: center;';
        
        const copyFacultyBtn = document.createElement('button');
        copyFacultyBtn.className = 'btn-sync';
        copyFacultyBtn.style.cssText = 'font-size: 0.85rem; padding: 4px 8px;';
        copyFacultyBtn.innerHTML = '<i class="fa-solid fa-copy"></i> Копіювати';
        copyFacultyBtn.onclick = (e) => {
            e.stopPropagation();
            const facultyLink = `${baseUrl}/dean.html?faculty=${faculty.code}`;
            copyLink(facultyLink);
        };
        
        const openFacultyBtn = document.createElement('button');
        openFacultyBtn.className = 'btn-sync';
        openFacultyBtn.style.cssText = 'font-size: 0.85rem; padding: 4px 8px; background: #3b82f6; color: white;';
        openFacultyBtn.innerHTML = '<i class="fa-solid fa-arrow-up-right-from-square"></i>';
        openFacultyBtn.title = 'Відкрити в новій вкладці';
        openFacultyBtn.onclick = (e) => {
            e.stopPropagation();
            const facultyLink = `${baseUrl}/dean.html?faculty=${faculty.code}`;
            window.open(facultyLink, '_blank');
        };
        
        const expandIcon = document.createElement('i');
        expandIcon.className = 'fa-solid fa-chevron-down';
        expandIcon.id = `${facultyId}-icon`;
        expandIcon.style.cssText = 'transition: transform 0.2s; color: #6b7280;';
        
        if (shouldExpand) {
            expandIcon.style.transform = 'rotate(180deg)';
        }
        
        facultyActions.appendChild(copyFacultyBtn);
        facultyActions.appendChild(openFacultyBtn);
        facultyActions.appendChild(expandIcon);
        facultyHeader.appendChild(facultyTitle);
        facultyHeader.appendChild(facultyActions);
        
        const departmentsContent = document.createElement('div');
        departmentsContent.id = `${facultyId}-content`;
        departmentsContent.style.cssText = shouldExpand 
        ...
        if (facultyDepartments.length > 0) {
            facultyDepartments.forEach(dept => {
                ...
                deptDiv.style.cssText = 'display: flex; justify-content: space-between; align-items: center; padding: 10px 15px 10px 35px; border-top: 1px solid #f3f4f6;';
                
                const deptName = document.createElement('span');
                deptName.style.cssText = 'font-size: 14px; color: #374151;';
                deptName.textContent = dept.name;
                
                const deptActions = document.createElement('div');
                deptActions.style.cssText = 'display: flex; gap: 6px;';
                
                const copyDeptBtn = document.createElement('button');
                copyDeptBtn.className = 'btn-sync';
                copyDeptBtn.style.cssText = 'font-size: 0.85rem; padding: 4px 8px;';
                copyDeptBtn.innerHTML = '<i class="fa-solid fa-copy"></i> Копіювати';
                copyDeptBtn.onclick = (e) => {
                    e.stopPropagation();
                    const deptLink = `${baseUrl}/department.html?department=${dept.code}`;
                    copyLink(deptLink);
                };
                
                const openDeptBtn = document.createElement('button');
                openDeptBtn.className = 'btn-sync';
                openDeptBtn.style.cssText = 'font-size: 0.85rem; padding: 4px 8px; background: #10b981; color: white;';
                openDeptBtn.innerHTML = '<i class="fa-solid fa-arrow-up-right-from-square"></i>';
                openDeptBtn.title = 'Відкрити в новій вкладці';
                openDeptBtn.onclick = (e) => {
                    e.stopPropagation();
                    const deptLink = `${baseUrl}/department.html?department=${dept.code}`;
                    window.open(deptLink, '_blank');
                };
                
                deptActions.appendChild(copyDeptBtn);
                deptActions.appendChild(openDeptBtn);
                
                deptDiv.appendChild(deptName);
                deptDiv.appendChild(deptActions);
                departmentsContent.appendChild(deptDiv);
            });
        } else {
            const noDeptMsg = document.createElement('div');
            noDeptMsg.style.cssText = 'padding: 10px 15px 10px 35px; color: #9ca3af; font-size: 13px; font-style: italic;';
            noDeptMsg.textContent = 'Немає кафедр';
            departmentsContent.appendChild(noDeptMsg);
        }
        
        facultyDiv.appendChild(facultyHeader);
        facultyDiv.appendChild(departmentsContent);
        container.appendChild(facultyDiv);
    });
}
function toggleFacultyAccordion(facultyId) {
    const content = document.getElementById(`${facultyId}-content`);
    const icon = document.getElementById(`${facultyId}-icon`);
...
function filterFacultiesAndDepartments() {
    const searchInput = document.getElementById('facultyDeptSearch');
    const searchTerm = searchInput ? searchInput.value : '';
    renderFacultiesAndDepartments(searchTerm);
}
